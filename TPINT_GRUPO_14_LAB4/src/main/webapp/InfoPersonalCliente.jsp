...
<body>
	<div class="sidebar">
		...
		<a href="Admin" class="menu-item">Panel Principal</a> <a
			href="Admin?opcion=clientes" class="menu-item active">Gestión de
			...
			de cuentas</a> <a href="ServletAlta" class="menu-item">Alta de
	...
	<div class="main-content">
		...
		<div class="content-area">
			<!-- Botón volver -->
			<a href="Admin?opcion=clientes" class="btn-back"> ← Volver a
				Lista de Clientes </a>
			...
			if ("ok".equals(baja)) {
			%>
			<div class="alert alert-success">✅ El cliente fue dado de baja
				exitosamente.</div>
			<%
			}
			%>
			...
			<div class="info-section">
				...
				<div class="info-grid">
					<div class="info-group">
						...
						<div class="info-row">
							<span class="info-label">Nombre:</span> <span class="info-value"><%=cliente.getNombre()%></span>
						</div>
						<div class="info-row">
							<span class="info-label">Apellido:</span> <span
								class="info-value"><%=cliente.getApellido()%></span>
						</div>
						<div class="info-row">
							<span class="info-label">DNI:</span> <span class="info-value"><%=cliente.getDNI()%></span>
						</div>
						<div class="info-row">
							<span class="info-label">Género:</span> <span class="info-value"><%=cliente.getGenero()%></span>
						</div>
					</div>
					...
					<div class="info-group">
						...
						<div class="info-row">
							<span class="info-label">Provincia:</span> <span
								class="info-value"><%=cliente.getProvincia().getNombre()%></span>
						</div>
						<div class="info-row">
							<span class="info-label">Localidad:</span> <span
								class="info-value"><%=cliente.getLocalidad().getNombre()%></span>
						</div>
						<div class="info-row">
							<span class="info-label">Dirección:</span> <span
								class="info-value"><%=cliente.getDireccion()%></span>
						</div>
					</div>
					...
					<div class="info-group">
						...
						<div class="info-row">
							<span class="info-label">Correo:</span> <span class="info-value"><%=cliente.getCorreo()%></span>
						</div>
						<div class="info-row">
							<span class="info-label">Teléfono:</span> <span
								class="info-value"><%=cliente.getTelefono()%></span>
						</div>
					</div>
				</div>

				<div class="button-group">
					<a href="EditarCliente?id=<%=cliente.getId()%>"
						...
						<div class="form-grid">
							...
							<div class="form-group">
								<label class="form-label">DNI:</label> <label
									class="form-control"
									style="background: #f8f9fa; color: #6b7280; cursor: default;">
									<%=cliente.getDNI()%>
								</label> <input type="hidden" name="dni" value="<%=cliente.getDNI()%>">
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								<label class="form-label">Provincia:</label> <select
									...
									<option value="<%=prov.getId()%>"
										<%=(request.getParameter("provincia") != null ? Integer.parseInt(request.getParameter("provincia")) == prov.getId()
				: cliente.getProvincia().getId() == prov.getId()) ? "selected" : ""%>>
										<%=prov.getNombre()%>
									</option>
									<%
									}
							...
							<div class="form-group">
								<label class="form-label">Localidad:</label> <select
									name="localidad" id="localidad" class="form-control" required>
									<%
									...
									<option value="<%=loc.getId()%>"
										<%=(cliente.getLocalidad().getId() == loc.getId()) ? "selected" : ""%>>
										<%=loc.getNombre()%>
									</option>
									<%
									}
					...
					<div class="button-group">
						<a href="Admin?opcion=detalle&id=<%=cliente.getId()%>&editar=no"
							class="btn btn-secondary"> ❌ Cancelar </a> <input type="submit"
							name="accion" value="Guardar Cambios" class="btn btn-success">
